...
class Database:
    ...
    def create_user(self, user) -> int:
        ...
        cursor.execute(
            ...
            INSERT INTO users (email, password_hash, name, user_type,
                               student_id, academic_year, major)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
    ...
    def save_profile(self, profile):
        ...
        cursor.execute(
            ...
            (user_id, required_courses, elective_courses, courses,
             interests, applications, rdia, weighting_mode, updated_at)
    ...
    def get_profile(self, user_id: int) -> Optional[Dict]:
        ...
        if profile:
            d = dict(profile)
            d["required_courses"] = self._safe_json_loads(d["required_courses"])
            d["elective_courses"]  = self._safe_json_loads(d["elective_courses"])
            d["courses"]           = self._safe_json_loads(d["courses"])
            d["interests"]         = self._safe_json_loads(d["interests"])
            d["applications"]      = self._safe_json_loads(d["applications"])
            return d
    ...
    def get_group(self, group_id: str) -> Optional[Dict]:
        ...
        if group:
            d = dict(group)
            d["members"] = self._safe_json_loads(d["members"])
            return d
        return None

    def get_user_group(self, user_id: int) -> Optional[Dict]:
        """
        Return the group that contains `user_id` as a member.

        FIX (Bug): This method was called in app.py (get_group, leave_group,
        finalize_group, get_recommendations, etc.) but was never defined in
        database.py, causing an AttributeError at runtime.

        We scan all groups and deserialize the JSON members list, then return
        the first group whose members list contains the given user_id.
        """
        conn   = self.get_connection()
        cursor = conn.cursor()
        ...
        for row in rows:
            d       = dict(row)
            members = self._safe_json_loads(d["members"])
    ...
    def update_group_members(self, group_id: str, members: List[int]):
        """
        Persist an updated members list for a group.

        FIX (Bug): Called in app.py (join_group, leave_group) but never
        defined in database.py, causing AttributeError at runtime.
        """
        conn   = self.get_connection()
        cursor = conn.cursor()
        cursor.execute(
    ...
    def delete_group(self, group_id: str):
        """
        Remove a group and its associated recommendations / weights.

        FIX (Bug): Called in app.py (leave_group) but never defined in
        database.py, causing AttributeError at runtime.
        """
        conn   = self.get_connection()
        cursor = conn.cursor()
        # Delete child rows first to respect foreign-key constraints
        cursor.execute(
            "DELETE FROM group_recommendations WHERE group_id = ?", (group_id,)
        )
        cursor.execute(
            "DELETE FROM group_weights WHERE group_id = ?", (group_id,)
        )
        cursor.execute(
    ...
    def finalize_group(self, group_id: str):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE groups SET is_finalized = 1 WHERE group_id = ?", (group_id,)
        )
        conn.commit()
        conn.close()
