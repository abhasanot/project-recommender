...
def grade_to_weight(grade) -> float:
    """
    Convert letter grade or numeric grade to embedding weight.
    Supports: "A+", "A", "B+", etc. or numeric values 0-4.5
    """
    # Handle numeric grades (GPA style)
...
def get_course_texts(course: dict, plos_map: Dict[str, str] = None) -> List[str]:
    ...
    Returns: [title+description, level, prerequisites, credit hours, clo1, clo2, ...]
    """
    segments = []
    ...
    # Each CLO statement with its associated PLO
    ...
    for category in ["knowledge", "skills", "values"]:
        for clo in clos.get(category, []):
            ...
            if stmt:  # Fixed: now correctly inside the loop
...
def get_project_segments(
    ...
    acm_texts = []
    for code in clf.get("acm", []):
        if code in acm_map:
            acm_texts.append(acm_map[code])
    ...
    # Domain labels with descriptions
    domain_parts = []
