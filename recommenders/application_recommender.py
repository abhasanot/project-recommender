...
Score all application domains for the group. No changes from original.
"""
import numpy as np
...
class ApplicationRecommender:
    ...
    def recommend(self, group_vec, group_meta, top_n=3) -> List[dict]:
        freq = self._domain_frequency(group_vec, "application")
        results = []
        for name, vec in self.engine.app_vecs.items():
