...
export default function ProfilePage() {
    ...
    if (validateProfile()) {
      try {
        const profileData = {
          ...
          courses: [...selectedElectiveCourses],
  ...
  const validateProfile = () => {
    if (selectedElectiveCourses.length === 0) {
      toast.error('Please add at least one elective course');
      return false;
    }
    if (selectedElectiveCourses.some(c => !c.grade)) {
      toast.error('Please assign grades to all elective courses');
      return false;
    }
